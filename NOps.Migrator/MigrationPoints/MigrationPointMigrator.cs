...
namespace NOps.Migrator.MigrationPoints
{
	public class MigrationPointMigrator
	{
			...
			lock (_migrationLock)
			{
				...
				while(migrationSequence.Any())
				{
					...
					try
					{
						...
						if(typeof(IMigrateDown).GetTypeInfo().IsAssignableFrom(migrationType.Type))
						{
		...
		{
			return (MigrationAttribute)t.GetTypeInfo().GetCustomAttributes(typeof(MigrationAttribute), false).SingleOrDefault();
		}
